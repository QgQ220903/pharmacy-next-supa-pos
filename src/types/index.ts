...
export interface ProductBatch {
  ...
  expiry_date: string;
  quantity: number;
  cost_price: number;   // ALTER TABLE: thêm sau, default 0
  created_at: string;
  updated_at: string;
}
...
export interface Sale {
  ...
  customer_phone?: string | null;  // Dùng trong sales/page.tsx
  total_amount: number;
  discount: number;                 // Dùng trong sales/page.tsx
  final_amount: number;
...
// Giá trị thực tế trong DB (trigger dùng 'purchase' và 'sale')
export type TransactionType =
  | 'purchase'         // Nhập kho (từ trigger trg_after_stock_entry_item)
  | 'sale'             // Bán hàng (từ trigger trg_after_sale_item)
  | 'return'           // Trả hàng
